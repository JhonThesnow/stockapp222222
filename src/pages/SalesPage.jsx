...
const SalesPage = () => {
    ...
    return (
        <div className="flex flex-col md:grid md:grid-cols-3 md:gap-6 h-full p-2 md:p-4 bg-gray-50">
            ...
            {/* PANEL DE PRODUCTOS */}
            <div className={`lg:col-span-2 bg-white p-2.5 md:p-4 rounded-xl shadow-md flex flex-col ${showCartOnMobile ? 'hidden' : 'flex'} md:flex`}>
                <div className="flex items-center justify-between mb-4 md:mb-6">
                    <h2 className="text-lg md:text-2xl font-bold text-gray-800">Productos</h2>
                    <button onClick={() => setShowQuickSale(true)} className="flex items-center gap-1 md:gap-2 text-xs md:text-sm font-semibold bg-purple-100 text-purple-700 py-1.5 px-3 md:py-2 md:px-4 rounded-xl hover:bg-purple-200 transition-colors">
                        <FiPlusCircle className="w-4 h-4 md:w-5 md:h-5" /> <span className="hidden sm:inline">Venta Rápida</span><span className="sm:hidden">Rápida</span>
                    </button>
                </div>
                ...
                <div className="flex justify-between items-center pt-3 md:pt-5 mt-3 md:mt-4 border-t-2">
                    ...
                    >
                        <FiChevronLeft className="w-4 h-4 md:w-5 md:h-5" /> <span className="hidden sm:inline">Anterior</span>
                    </button>
                    <span className="font-bold text-sm md:text-lg text-gray-700">Pág {currentPage} de {totalPages}</span>
                    <button
                        onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))}
                        disabled={currentPage === totalPages}
                        className="p-2 md:py-3 md:px-6 rounded-xl bg-gray-200 text-gray-800 font-bold hover:bg-gray-300 disabled:opacity-50 flex items-center gap-1 md:gap-2"
                    >
                        <span className="hidden sm:inline">Siguiente</span> <FiChevronRight className="w-4 h-4 md:w-5 md:h-5" />
                    </button>
                </div>
            </div>

            {/* PANEL DE CARRITO */}
            <div className={`bg-white p-2.5 md:p-4 rounded-xl shadow-md flex flex-col ${!showCartOnMobile ? 'hidden' : 'flex'} md:flex border-l-0 md:border-l border-blue-100 h-full`}>
                <div className="flex items-center justify-between border-b-2 pb-3 md:pb-4 mb-3 md:mb-4 bg-white sticky top-0">
                    ...
                    <h2 className="text-lg md:text-2xl font-bold text-center flex-grow text-gray-800">Carrito</h2>
                    ...
                    {cart.length > 0 ? cart.map(item => (
                        <div key={item.id} className="flex items-center gap-2 md:gap-3 mb-3 md:mb-5 p-2 md:p-3 bg-gray-50 rounded-lg border">
                            ...
                            <div className="flex items-center gap-1 md:gap-3 bg-white border rounded-xl p-1 shadow-sm flex-shrink-0">
                                ...
                                <span className="w-6 md:w-8 text-center font-bold text-sm md:text-lg">{item.quantity}</span>
                            ...
                            <button onClick={() => removeItemFromCart(item.id)} className="text-red-500 p-1 md:p-2 hover:bg-red-100 rounded-lg flex-shrink-0"><FiXCircle className="w-5 h-5 md:w-6 md:h-6" /></button>
                        </div>
                    )) : (
                        <div className="text-center pt-10 md:pt-20 text-gray-400 flex flex-col items-center">
                            <FiShoppingCart className="w-12 h-12 md:w-16 md:h-16 mb-4 md:mb-6 opacity-50" />
                            <p className="text-lg md:text-xl font-medium">El carrito está vacío</p>
                ...
                <div className="border-t-2 pt-4 md:pt-5 mt-2 md:mt-4">
                    <div className="mb-4 md:mb-5">
                        ...
                        <select value={currentPaymentMethod || ''} onChange={(e) => setCurrentPaymentMethod(e.target.value)} className="p-2 md:p-3 border rounded-xl bg-white w-full text-sm md:text-lg font-medium">
                    ...
                    {/* TOTAL ACTUALIZADO */}
                    <div className="flex justify-between items-center mb-4 md:mb-6 bg-gray-100 p-2 md:p-3 rounded-xl">
                        ...
                        <div className="text-right flex flex-col items-end">
                            {currentPaymentMethod === 'Efectivo' && (
                                <span className="text-xs md:text-sm text-gray-500 line-through mb-1">
